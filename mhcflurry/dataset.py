...
class Dataset(object):
    ...
    def from_sql_pan(
        ...
        query_string_rd = 'SELECT MHCseq FROM ElutionArray WHERE MHCseq != ? \
            ORDER BY RANDOM() LIMIT 25000'
        mhcSeqPool = pd.read_sql_query(query_string_rd, engine, params=["NO_PSEUDO"])
        # Attach to RD peptide sequences
        df_neg = pd.read_sql_query(query_string, engine, params=["RD",length])
