...
class Dataset(object):
    ...
    def from_sql(
        ...
        df_neg = df_neg.sample(n=subset, replace=True)
    ...
    def from_sql_pan(
            ...
            length=9):
        """
        Get Dataset from HLA-peptidome.db with Pan-training
        Training set is a ensemble of all hla types with training data > 50
        Negative samples, generated by random sequences, are assigned with random MHCseq
        Assign negative sample with random weight
        """
        # Draw a set of random mhcseq from db
        query_string_mhc = 'SELECT MHCseq FROM ElutionArray WHERE MHCseq != ? \
            ORDER BY RANDOM() LIMIT 25000'
        mhcSeqPool = pd.read_sql_query(query_string_mhc, engine, params=["NO_PSEUDO"])

        query_string = 'SELECT peptide, unique_allele, MS_score, MHCseq FROM ElutionArray \
            WHERE unique_allele = ? AND length = ?'

        df_pos = pd.DataFrame()
        df_neg = pd.DataFrame()
        # Draw allele-specific df from both positive and negative data
        for allele in alleles:
            df_pos_allele = pd.read_sql_query(query_string, engine, params=[str(allele),length])
            df_pos = pd.concat([df_pos, df_pos_allele], axis=0)
            df_neg_allele = pd.read_sql_query(query_string, engine, params=["RD",length])
            df_neg_allele['unique_allele'] = allele
            df_neg = pd.concat([df_neg, df_neg_allele], axis=0)            

        df_pos = df_pos.sample(n=pos_count,replace=True,axis=0)
        df_pos['peptide'] = df_pos['peptide'].map(str) + df_pos['MHCseq']
        
        df_pos.rename(columns = {'MS_score':'affinity'}, inplace = True)
        df_pos['affinity'].fillna(0.9, axis = 0, inplace = True)
        df_pos['sample_weight'] = 1

        # Attach MHCseq to RD peptide sequences
        df_neg = df_neg.sample(n=25000,replace=True,axis=0)
        assert len(df_neg) == len(mhcSeqPool), "length is not equal: %d\t%d" % (len(df_neg),
            len(mhcSeqPool))
        df_neg.reset_index(drop=True, inplace=True)
        df_neg['peptide'] = df_neg['peptide'].map(str) + mhcSeqPool['MHCseq']
   
        df_neg.rename(columns = {'MS_score':'affinity'}, inplace = True)
        df_neg.replace(to_replace={'affinity': {1: 0}}, inplace=True)
        df_neg['affinity'].fillna(0, axis = 0, inplace = True)
        df_neg['sample_weight'] = np.random.rand(25000)*neg_sw
